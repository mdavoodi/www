...
We are proud to release [Vault 0.2](https://vaultproject.io). Vault is a
tool for managing secrets. From storing credentials and API keys to
encrypting sensitive data, Vault is meant to be a solution for all secret
management needs.

The initial public release of Vault was almost two months ago and we've
...
keys for the transit backend among many other features.

Vault 0.2 can be [downloaded here](https://vaultproject.io/downloads.html),
and the full changelog can be [viewed here](https://github.com/hashicorp/vault/blob/v0.2.0/CHANGELOG.md).

Read on to learn more about the major new features in 0.2.

READMORE

## Key Rotation and Rekeying

When Vault is first started, the `vault init` command is used to initialize
the Vault. This creates the encryption key which is used to protect data at rest,
the master key which is used to protect the encryption key, and key shares which
are used to split the master key.

With Vault 0.1, all of those were static and could not be changed after they
...
to protect data at rest. When it is invoked, either through the CLI or directly
...
The new [`pki` secret backend](https://vaultproject.io/docs/secrets/pki/index.html)
is used to turn Vault into an internal certificate authority. The backend makes use
...
A major thanks to Jeff Mitchell for contributing this backend and
to Akamai for sponsoring his efforts.

## Per-Transaction Derived Keys

The [`transit` secret backend](https://vaultproject.io/docs/secrets/transit/index.html)
has been available since release. The backend is used to help encrypt and decrypt
data in transit through Vault. We've [previously blogged](/blog/how-atlas-uses-vault-for-managing-secrets.html)
about how it is used to protect the sensitive data stored by Atlas.
...
ever having access to the encryption keys used. This makes it easier to manage
...
of a named key. When set, any encrypt or decrypt operation must also provide a
`context` along with the plaintext or ciphertext.
...
now be denied where it was previously allowed by the first rule.
...
If you experience any issues, please report them on [GitHub](https://github.com/hashicorp/vault/issues).
