...
in an easy-to-deploy single Go binary. It is distributed, highly available, and
proven to scale to thousands of nodes and services across multiple datacenters.
...
In addition to network tomography and the new in-memory database, Consul 0.6
also includes other new features like prepared queries, enhanced ACLs, TCP and
Docker container health checks, and better low-level network robustness. Consul
0.6 is also now 100% pure Go, making it even easier to build and deploy.
...
provided by Serf, has all nodes in a cluster perform random probes of other nodes
