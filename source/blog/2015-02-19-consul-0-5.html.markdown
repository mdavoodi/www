...
Consul 0.5 brings many new features including Atlas integration,
enhanced ACLs, simple N+1 deploys, node and service maintenance modes,
...
a commercial platform offering by HashiCorp. Atlas provides a single platform
to deploy any application to any infrastructure. Integration with Atlas is entirely optional,
but it provides new functionality that is supported by Atlas.
...
These features are included with every Atlas account, and are available now.
Future versions of the Atlas UI will integrate Consul health checks with
