...
datacenter runtime, providing many of the capabilities needed to run
a modern datacenter: service discovery, configuration, and orchestration.
...
This release brings many new features including Atlas integration,
enhanced ACLs, simple N+1 deploys, node and service maintanence modes,
native HTTP health checks, ephemeral keys, session TTLs, and key rotation
amoung many others.

Consul 0.5 can be [downloaded here](http://www.consul.io/downloads.html),
and the full changelog can be [viewed here](https://github.com/hashicorp/consul/blob/v0.5.0/CHANGELOG.md).
...
Consul 0.5 is the first release that integrates with [Atlas](https://atlas.hashicorp.com),
which is a commerical offering by HashiCorp. Atlas provides a single platform
to deploy any application to any infrastructure. Integration with Atlas is optional,
and provides new functionality that is supported by Atlas.

Bootstrapping a Consul cluster has always required providing a well-known IP
or DNS address so that an agent can join an existing cluster. This can be solved
in a number of ways, but Atlas now provides an auto-join mechanism. Using Atlas
auto-join is extremely easy, and removes the burden of maintaining a well known
address.
...
as well as to link server together over the WAN in a global cluster.

In addition to providing an auto-join feature, linking a Consul cluster to Atlas
allows cluster state to be displayed in the Atlas UI:

![Atlas Consul UI](/images/blog/consul/atlas-consul-ui.png)

These features of Atlas are free, and do not require a paid account and are available
now. Future versions of the Atlas UI will integrate Consul health checks with
alerting so that you can notify PagerDuty, send an email or receive an SMS message.
...
Consul 0.4 was the first version to introduce ACL support. Initially, ACLs
were able to enforce access to the Key/Value store, but with Consul 0.5
initial support is now available for protecting service discovery and
registration.
...
With Consul 0.5, the policy language supports specify read level
...
register a service, any any level of privilege can be used for discovery.
...
However, integrating applications to be Consul-aware and to implement leader election
...
we want to deploy it on at least 2 machines, but ensure no more than one instance is running.
...
instance is running. However, we may actually want to have more than one instance
of `foo` running at a time. This is done by specifying the limit of running instances:
...
using Consul `lock` makes it trivial to do an N+1 style deployment, allowing Consul to
manage the complexity so that applications can be unaware.

More details are available in the documentation for the [`lock` documentation](https://consul.io/docs/commands/lock.html).

## Maintanence Modes

A common request with Consul has been the ability to put a node or service
into a maintanence mode. This can be used to decommision a node, or to
...
As an example, to toggle node maintanence mode:
...
More details are available in the documentation for the [`maint` documentation](https://consul.io/docs/commands/maint.html).
...
With Consul 0.5, there is now native HTTP health checks. Doing REST based
micro-services is a very common pattern, and using HTTP health checks
...
Consul has supported encryption for the gossip protocol but there was way to
support multiple keys or key rotation. Inspired by the equivalent in Serf,
...
More details are available in the documentation for the [`keyring` documentation](https://consul.io/docs/commands/keyring.html).
...
There is a 15 minute window where a Consul 0.5 leader server can be run without
issues providing an opportunity to do a rolling upgrade without issue. Alternatively,
if the follower nodes are upgraded first and the leader last, the upgrade will
be seamless.
